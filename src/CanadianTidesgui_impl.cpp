...
void Dlg::DrawAllStationIcons(PlugIn_ViewPort *BBox, bool bRebuildSelList,
  ...
  for (std::list<myPort>::iterator it = myports.begin(); it != myports.end();
       it++) {
    plat = (*it).coordLat;
    plon = (*it).coordLon;
...
void Dlg::DrawAllSavedStationIcons(PlugIn_ViewPort *BBox, bool bRebuildSelList,
  ...
  for (list<myPort>::iterator it = mySavedPorts.begin();
       it != mySavedPorts.end(); it++) {
    plat = (*it).coordLat;
    plon = (*it).coordLon;
    outPort.Name = (*it).Name;
...
void Dlg::OnDownload(wxCommandEvent &event) {
  ...
  int region = m_choice31->GetCurrentSelection();
  wxString choiceRegion = m_choice31->GetString(region);

  wxString urlString =
      "https://api-iwls.dfo-mpo.gc.ca/api/v1/stations?chs-region-code=" +
      choiceRegion + "&&time-series-code=wlp-hilo";
  wxURI url(urlString);
...
void Dlg::OnGetSavedTides(wxCommandEvent &event) {
  ...
  if (mySavedPorts.size() == 0) {
    wxMessageBox(
        _("No locations are available, please download and select a tidal "
          "station"));
    return;
  }
  ...
  GetTidalEventDialog *GetPortDialog = new GetTidalEventDialog(
      this, -1, _("Select the Location"), wxPoint(200, 200), wxSize(300, 200),
      wxRESIZE_BORDER);
  ...
  if (GetPortDialog->ShowModal() != wxID_OK) {
    ...
    for (;;) {
      itemIndex = GetPortDialog->dialogText->GetNextItem(
    ...
    if (foundPort) {
      // Set what row it is (m_itemId is a member of the regular wxListCtrl
      // class)
...
void Dlg::getHWLW(string id) {
  ...
  wxString urlString = "https://api-iwls.dfo-mpo.gc.ca/api/v1/stations/" + id +
                       tidalevents + code + fromDate + snow + toDate + snowplus;
  wxURI url(urlString);

  wxString tmp_file = wxFileName::CreateTempFileName("");

  _OCPN_DLStatus ret =
      OCPN_downloadFile(url.BuildURI(), tmp_file, "", "", wxNullBitmap, this,
  ...
  bool parsingSuccessful = reader->parse(
      myjson.c_str(), myjson.c_str() + myjson.size(), &root2, &errors);
  delete reader;

  if (!parsingSuccessful) {
    wxLogMessage("Parsing error");
    return;
  }
  ...
  for (std::list<myPort>::iterator it = mySavedPorts.begin();
       it != mySavedPorts.end();) {
    if ((*it).Id == id) {
...
void Dlg::OnShow(void) {
  tidetable =
      new TideTable(this, 7000, _("Tides"), wxPoint(200, 200), wxSize(-1, -1),
                    wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER);
  wxString label =
      m_titlePortName + _("      (Times are UTC)  ") + _(" (Height in metres)");
  ...
  if (myevents.empty()) {
    wxMessageBox(
        _("No tidal data found. Please use right click to select the Canadian "
          "tidal station"));
    return;
  }
...
void Dlg::OnShowSavedPortTides(wxString thisPortId) {
  if (mySavedPorts.empty()) {
    wxMessageBox(
        _("No tidal data found. Please download the locations \n and use right "
          "click to select the Canadian tidal station"));
    return;
  }

  tidetable =
      new TideTable(this, 7000, _("Locations Saved"), wxPoint(200, 200),
  ...
  for (std::list<myPort>::iterator it = mySavedPorts.begin();
    ...
    if ((*it).Id == thisPortId) {
      ...
      wxString label = m_titlePortTides + _("      (Times are UTC)  ") +
                       _(" (Height in metres)");
      tidetable->itemStaticBoxSizer14Static->SetLabel(label);
      ...
      for (list<TidalEvent>::iterator itt = savedevents.begin();
           itt != savedevents.end(); itt++) {
        Event = (*itt).EventType;
...
void Dlg::getPort(double m_lat, double m_lon) {
  ...
  if (myports.empty()) {
    wxMessageBox(
        _("No active tidal stations found. Please download the locations"));
  }
...
wxString Dlg::getSavedPortId(double m_lat, double m_lon) {
  ...
  if (mySavedPorts.empty()) {
    wxMessageBox(
        _("No tidal stations found. Please download locations when online"));
...
void Dlg::OnClose(wxCloseEvent &event) { plugin->OnCanadianTidesDialogClose(); }
...
void Dlg::SaveTidalEventsToXml(list<myPort> myPorts) {
  ...
  for (list<myPort>::iterator it = myPorts.begin(); it != myPorts.end(); it++) {
    ...
    for (list<TidalEvent>::iterator it = myevents.begin(); it != myevents.end();
         it++) {
  ...
  if (!doc.SaveFile(filename))
    wxLogMessage(_("CanadianTides") + wxString(": ") +
                 _("Failed to save xml file: ") + filename);
}
...
GetTidalEventDialog::GetTidalEventDialog(wxWindow *parent, wxWindowID id,
